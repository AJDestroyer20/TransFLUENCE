...
class FLStudioWriter:
    ...
    def __init__(self):
        self.ppq = 96  # FL Studio default PPQ
    ...
    def _build_flp_binary(self, transproj: TransProj) -> bytes:
        ...
        if transproj.name:
            ...
            self._write_string(data, transproj.name)
    ...
    def _write_string(self, data: bytearray, text: str):
        """Write string with varint length (for events like VERSION, TITLE)"""
    ...
    def _write_channel(self, data: bytearray, idx: int, track: Track):
        ...
        if track.instrument:
            fl_plugin = PluginMapper.ableton_to_fl(track.instrument.name, is_instrument=True)
            plugin_state = self._generate_plugin_state(fl_plugin, track, is_instrument=True)
    ...
    def _generate_plugin_state(self, fl_plugin_name: str, track: Track, is_instrument: bool) -> bytes:
        """
        ...
        return PluginStateFactory.generate(
            fl_plugin_name,
            params={'volume': track.volume, 'pan': track.pan}
        )
    ...
    def _write_pattern(self, data: bytearray, pattern_idx: int, track_idx: int, 
        ...
        self._write_string(data, pattern_name)  # Use string writer
        ...
        for note in clip.notes:
            ...
            note_bytes[17] = int(note.velocity * 100)           # 17: Velocity (0-100)
